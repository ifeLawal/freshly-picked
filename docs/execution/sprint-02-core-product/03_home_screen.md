...
## Requirements
- Home screen created and registered in the tab navigator
- Recommendations fetched from `GET /api/v1/recommendations` using React Query
- Each recommendation rendered as a card showing summary info (title, host, category)
- Loading state displayed while fetch is in progress
- Empty state displayed when no recommendations are returned
